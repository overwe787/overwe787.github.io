...
//远程异步读取三个大型单词文本
    fetch('cet/cet61.txt') //读取cet/cet61.txt
    .then(resp => resp.text())
    .then(txt => {
        Model.CET6 = [] ;
        createCET6(txt) ;
        UI.log01('系统成功读取了'+ Model.CET6.length +'个单词！') ;
    }) ;
    setTimeout(function(){ //读取cet/cet62.txt
        fetch('cet/cet62.txt') //读取cet/cet62.txt
        .then(resp => resp.text())
        .then(txt => {
         createCET6(txt) ;
        UI.log01('系统成功读取了'+ Model.CET6.length +'个单词！') ;
    ...
    setTimeout(function(){
        ...
        .then(txt => {
            createCET6(txt) ;
            UI.log01('系统最后成功读取了'+ Model.CET6.length +'个单词！') ;
        } );
    } , 5 * 1000) ;
 ...
 let UI = {} ; //UI用于表达用户界面，以及改变用户界面上的的内容
   UI.printWord = function(){ //用于把当前单词（Model.pos存储的索引）显示出来
       //让全局变量与局部变量联系起来
        ...
        UI.log('正在学习第 '+ (pos+1) +'/'+Model.numOfLearning+ ' 个单词');

      } ;
   
 UI.log01 = function(s){
    select('p#foot').textContent = s ;
 };
